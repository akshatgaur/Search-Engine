    ...
    for (int i = q.args.size() - 1; i >= 0; i--) {
      ...
      if (q_i_after == null) {
        ...
        if (q_i_before != q_i_after) {
          float weight = q_i_before.getWeight();
          q_i_after.setWeight(weight);
  ...
  private static Qry parseString (String queryString)
    ...
    while (queryString.length() > 0) {
      ...
      //  STUDENT HW2 CODE GOES HERE
        // If the operator is wieghted operator get the weight of the terms
      float weight = 0.0f;
      if (queryTree instanceof QrySopWAnd || queryTree instanceof QrySopWSum ){

          PopData<String,String> p = popTerm(queryString);
          weight = Float.parseFloat(p.getPopped());
    ...
    // add the weight for the terms in the arg list
	queryTree.appendArg (qargs[i]);
	queryTree.args.get( queryTree.args.size() - 1 ).setWeight(weight);
      }
    }
